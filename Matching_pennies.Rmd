...
self_similar_priors <-
  list(mean_basic = 0, #agnostic
                   ...
                   mean = 0, #agnostic
                   variance = c(0,0), #will be exponated, so exp(0) = 1
                   param_mean = c(-2 , -1), #not agnostic. These will be exponated before being used as parameters. So this is a prior of exp(0) = 1
...
#Set opponent
agent_dfVS <- create_agents(c("PlaySelf", "1-ToM"), match_up = "RR", prepare_with_defaults = T)
#Run game
result_df_trial1 <- compete(player1 = agent_dfVS$player1,
...
result1 <- result_df_trial1 %>% filter(player == "PlaySelf")
P11_1_1 <- sum(result1$points)
...
result_df_trial2 <- compete(player1 = agent_dfVS$player1,
              player2 = agent_dfVS$player2,
              p1_params = agent_dfVS$params_p1,
              p2_params = agent_dfVS$params_p2,
              p1_hidden_states = agent_dfVS$hidden_states_p1,
...
result2 <- result_df_trial2 %>% filter(player == "PlaySelf")
P11_1_2 <- sum(result2$points)

```

```{r 3_TOM}
#Set opponent
agent_dfVS <- create_agents(c("PlaySelf", "3-ToM"), match_up = "RR", prepare_with_defaults = T)

result_df_trial3 <- compete(player1 = agent_dfVS$player1,
              player2 = agent_dfVS$player2,
              p1_params = agent_dfVS$params_p1,
              p2_params = agent_dfVS$params_p2,
              p1_hidden_states = agent_dfVS$hidden_states_p1,
...
result3 <- result_df_trial3 %>% filter(player == "PlaySelf")
P11_1_3 <- sum(result3$points)
ID_11_1 <- sum(P11_1_1 + P11_1_2 + P11_1_3)
...
result3 <- result_df_trial3 %>% filter(player == "PlaySelf")
P11_2_3 <- sum(result3$points)
ID_11_2 <- sum(P11_2_1 + P11_2_2 + P11_2_3)
```
